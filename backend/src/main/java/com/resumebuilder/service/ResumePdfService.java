...
public class ResumePdfService {
    ...
    public byte[] generateResume(Resume resume) throws Exception {
        ...
        if (resume.getSkillCategories() != null && !resume.getSkillCategories().isEmpty()) {
            ...
            addSkillCategories(document, resume.getSkillCategories(), normalFont, boldFont);
        }
    ...
    private void addSection(Document document, String title, Font sectionFont) throws Exception {
    ...
    private void addSummary(Document document, String summary, Font normal) throws Exception {
    ...
    private void addExperience(Document document, List<Experience> experiences, Font normal, Font bold) throws Exception {
        for (Experience exp : experiences) {
            ...
            if (exp.getResponsibilities() != null) {
                com.lowagie.text.List bulletList = new com.lowagie.text.List(false, 10f);
    ...
    private void addSkillCategories(Document document, List<SkillCategory> categories, Font normal, Font bold) throws Exception {
        ...
        for (SkillCategory category : categories) {
            ...
            paragraph.add(new Chunk(safe(category.getCategoryName()) + ": ", bold));
            paragraph.add(new Chunk(String.join(", ", category.getSkills()), normal));
            document.add(paragraph);
        }
    }
    ...
    private void addCertifications(Document document, List<Certification> certifications, Font normal, Font bold) throws Exception {
        ...
        for (Certification cert : certifications) {
            ...
            phrase.add(new Chunk("  ", normal));
            if (cert.getCertificateUrl() != null && !cert.getCertificateUrl().isBlank()) {
    ...
    private BaseFont loadFont(String path) throws Exception {
        ...
        if (fontUrl == null) throw new RuntimeException("Font not found at " + path);
        return BaseFont.createFont(fontUrl.toString(), BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
    }
}
